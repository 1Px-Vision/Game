...
fn fn_url(url:&str)-> Result<String,reqwest::Error>{
    let response=blocking::get(url)?;
    let text=response.text()?;
    return Ok(text);
}

fn fn_sprinte()->Result<Sprite,Box<dyn std::error::Error>>{
    let url = "https://get-random-sprite-data-dan-chiarlones-projects.vercel.app/api/handler";
    let json_code: &str = &fn_url(url)?;
    let sprite: Sprite = serde_json::from_str(json_code)?;
    return Ok(sprite);
}

fn fn_sprites_loop(sprite_sender: std::sync::mpsc::Sender<Sprite>, end_receiver: std::sync::mpsc::Receiver<bool>){
    loop {
        let result=fn_sprinte();
        match result{
...
fn fn_randn(n:i32)->u16{
    return rand::thread_rng().gen_range(0..n+1) as u16;
}

fn fn_draw_sprites(sprites: &Vec<Sprite>) {
...
fn main() {
    ...
    let networking_thread = std::thread::spawn(|| {fn_sprites_loop(sprite_sender, end_receiver);});
    let title=CString::new("Project Videogame").unwrap();
    let mut sprinte_count=0;
    ...
    opengl_game::start_window_and_game_loop!(title.as_ptr(), 800, 600, {
      ...
      fn_draw_sprites(&queue);
      if let Ok(sprite)=sprite_receiver.try_recv(){
        sprinte_count+=1;
        println!("{}:{}",sprinte_count,sprite);
        queue.push(sprite);
        if queue.len()>4 {queue.remove(0);}
      }
      if opengl_game::opengl_ffi::rust_window_should_close() == 1 || opengl_game::opengl_ffi::rust_get_key(opengl_game::opengl_ffi::rust_get_window(), opengl_game::opengl_ffi::GLFW_KEY_ESCAPE) == opengl_game::opengl_ffi::GLFW_PRESS {
